...
/*
    Programa que borra un directorio, llamando a la función mi_unlink() de la capa de directorios.
...
int main(int argc, char* argv[]){
    ...
    if (strlen(argv[2])== 1 && argv[2][0] == '/') { // Comprobamos que no sea el directorio raiz
        fprintf(stderr, ROJO"Error: No se puede eliminar el directorio raiz."RES);
        return FALLO;
    }
    ...
    if(err == FALLO){
        ...
        fprintf(stderr, ROJO"Error: El directorio %s no esta vacio."RES, argv[2]);
    }
