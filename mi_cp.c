...
/*
    ...
    Sintaxis: ./mi_cp <disco> </origen/nombre/> </destino/>
*/
int main(int argc, char *argv[]){
    ...
    int err = mi_cp(argv[3], argv[2], true);
    switch(err) {
        case -1:
            fprintf(stderr,ROJO"Se ha producido un error durante la operacion de copiado.\n"RES);
            break;
        case -2:
            fprintf(stderr, ROJO"Error: %s no es un directorio. Usa mi_cp_f para copiar ficheros.\n"RES, argv[2]);
            break;
        case -3: 
